%% 数值代数大作业 第三问(Inexact Uzawa方法求解Stokes方程）
% 设置参数处调整合适的参数后单击运行即可
%% 设置参数
level = 9; alpha = 1; tau = 2; % 网格规模 N=2^level; 迭代法参数 alpha; 近似解精度要求 tau
r = 8; nu1 = 3; nu2 = 3; % 网格层数 r+1(L=2^r); 前磨光次数 nu1; 后磨光次数 nu2
%% 设置方程组常数项与真解
...
res = r0; cycle = 0; % 当前残量2范数 res; 循环次数 cycle
while res/r0 > 10^(-8)
    cycle = cycle + 1;
    [U,V] = PCGforStokes(U,V,P,F,G,tau,level,r,nu1,nu2); % 预优共轭梯度法求解子问题
    D_err = residueForP(U,V,D); % 计算B^TU
    P = P + alpha * D_err; % 更新 P
    [F_err,G_err,D_err] = residue(U,V,P,F,G,D); % 计算残量
    res = sqrt(sum(sum(F_err.^2))+sum(sum(G_err.^2))+sum(sum(D_err.^2)));
    disp(['InexactUzawa complete ',num2str(cycle),' times, with residue ',num2str(res)])
end
%% 误差计算
